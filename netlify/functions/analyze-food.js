...
const PROMPT = `התבונן בתמונה בעיון וזהה רק מזון שבאמת נראה בה בפועל. אם התמונה מציגה מזון ארוז/ממותג (כמו חטיף, סוכרייה, שוקולד) גם ללא תווית תזונה גלויה — תן הערכה סבירה על סמך ערכים תזונתיים טיפוסיים למוצר דומה או לקטגוריה שלו (לדוגמה: סוכריה קשה בודדת כ-20-25 קלוריות), ואל תסרב במקרה כזה. אך אם התמונה אינה ברורה, פגומה, ריקה, או שאינך רואה בה מזון כלל — אסור לך להמציא פריט מזון שאינו נראה בפועל; במקרה כזה החזר מערך items ריק ואל תכלול שדה rating. עבור כל פריט אמיתי שזיהית, הערך גודל מנה סביר בגרמים וחשב קלוריות, חלבון, פחמימות, שומן וסיבים תזונתיים למנה הזו. בנוסף, דרג את איכות הארוחה כמכלול (על סמך מה שאתה רואה בפועל — מזון טבעי/שלם לעומת מעובד, איזון תזונתי, כמות ירקות) בציון מ-1 עד 10 (10=הכי בריא), עם תווית קצרה ("בריא מאוד"/"מאוזן"/"פחות מומלץ" וכו') והערה קצרה של משפט אחד. השב אך ורק ב-JSON תקין בפורמט המדויק הזה: {"items":[{"name":"שם המזון בעברית","grams":מספר,"kcal":מספר,"protein":מספר,"carbs":מספר,"fat":מספר,"fiber":מספר}],"rating":{"score":מספר,"label":"תווית קצרה","note":"הערה קצרה"}}`;
